    ...
    <section className="dot-background">
      ...
      <div className="projects-grid">
        {projects.map((project) => (
          <div   className={`project-card ${  featuredProjects[0] === project.name ? "featured" : ""   }`}   key={project.id}  >
            <div>
              <h3>{project.name}</h3>

              <p>
                {project.description  ? project.description: "Production-ready application demonstrating clean architecture and scalable design."}
              </p>

              <div className="tech-tags">  {(techMap[project.name] || []).map((tech) => (
            ...
            <div className="project-actions">
              ...
              <a className="code-btn"   href={project.html_url}  target="_blank"  rel="noopener noreferrer"> View Code
              </a>
            </div>
          </div>
        ))}
